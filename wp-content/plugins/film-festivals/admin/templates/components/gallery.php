...
foreach($this->gallery() as $k => $item): ?>
    <div class="gallery-item gallery-item-<?=$k;?>">
        <div class="gallery-item__image">
            ...
            <button type="button" class="upload_image_button button"><?= __( 'Upload', $this->plugin_name ); ?></button>
            <button type="button" class="remove_image_button button">&times;</button></div>
        </div>
        <div class="gallery-item__meta">
            <label for="<?=$this->plugin_name; ?>_image[<?=$k;?>][order]"><?= __( 'Slide order', $this->plugin_name ); ?></label>
            <input type="number" name="<?=$this->plugin_name; ?>_image[<?=$k;?>][order]" id="<?=$this->plugin_name; ?>_image[<?=$k;?>][order]" value="<?= $item['order']??null; ?>" />
            <label for="<?=$this->plugin_name; ?>_image[<?=$k;?>][title]"><?= __( 'Slide caption', $this->plugin_name ); ?></label>
...
<button type="button" class="add-gallery-item button"><?= __( 'Add slide', $this->plugin_name ); ?></button>
    ...
    $(document).ready(function() {
        $(".add-gallery-item").click(function() {
            ...
            var item = '<div class="gallery-item gallery-item-'+count+'">'+
                ...
                '</div>'+
                '<div class="gallery-item__meta">'+
                    '<label for="<?=$this->plugin_name; ?>_image['+count+'][order]"><?= __( 'Slide order', $this->plugin_name ); ?></label>'+
                    '<input type="number" name="<?=$this->plugin_name; ?>_image['+count+'][order]" id="<?=$this->plugin_name; ?>_image['+count+'][order]" value="" />'+
                    '<label for="<?=$this->plugin_name; ?>_image['+count+'][title]"><?= __( 'Slide caption', $this->plugin_name ); ?></label>'+
