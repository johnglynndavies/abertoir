...
<select id="slider_select" name="<?=$this->plugin_name; ?>_name[sliderselect]">
    ...
    <?php foreach ($this->main_events() as $id => $category): ?>
        <option value="<?= $id; ?>"<?php echo (!empty($this->options['sliderselect']) && $this->options['sliderselect'] == $id ? ' selected' : ''); ?>><?= $category; ?></option>
    <?php endforeach; ?>
</select>
